...
const restrictValue = id => {
    const fields = ["fOneAtk", "fOneDef", "fOneSpd"].includes(id) ? ["fOneAtk", "fOneDef", "fOneSpd"] : ["fTwoAtk", "fTwoDef", "fTwoSpd"];
    const max = 100;
...
getFightStats = PlayerOne => {
    const [atk, def, spd] = PlayerOne ? ["fOneAtk", "fOneDef", "fOneSpd"] : ["fTwoAtk", "fTwoDef", "fTwoSpd"];
    return {
...
const startBattle = () => {
    ...
    const fighterOne = new Fighter ("fighterOne", "fighterOneHp", fighterOneStats.atk, fighterOneStats.def, fighterOneStats.spd, "Klebertz")
    const fighterTwo = new Fighter ("fighterTwo", "fighterTwoHp", fighterTwoStats.atk, fighterTwoStats.def, fighterTwoStats.spd, "Gumers")
...
class Fighter {
    ...
    GotHit(){
        getById(this.id === fighterOne ? fighterTwo : fighterOne).classList.add(oopClassList.fighterHit);
    }
    Dodge(){
        getById(this.id === fighterOne ? fighterTwo : fighterOne).classList.add(oopClassList.miss);
    }
    ResetPosition(){
        setTimeout(() => {
            getById(this.id).classList.remove(oopClassList.attack);
            getById(this.id === fighterOne ? fighterTwo : fighterOne).classList.remove(oopClassList.fighterHit);
            getById(this.id === fighterOne ? fighterTwo : fighterOne).classList.remove(oopClassList.miss);
        }, 1000);
    }
    AnimateHit(){
        this.AtkPosition();
